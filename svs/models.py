...
class PlayerRecord(TypedDict, total=False):
    ...
    Core fields (always present):
        User ID, Level, Construction, Research, Troops, FCs, FC Shards, Time UTC, Days

    Confidence metadata (one per editable field):
        _conf_Level, _conf_Construction, _conf_Research, _conf_Troops,
        _conf_FCs, _conf_FC Shards, _conf_Time UTC, _conf_Days

    Warning metadata (present only when confidence < HIGH):
        _warn_Time UTC, etc.

    Internal metadata:
        _fc_raw   : raw FC/shards line seen by the parser
